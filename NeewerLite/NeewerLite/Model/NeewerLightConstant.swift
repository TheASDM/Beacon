...
class NeewerLightConstant {
    ...
    class func getLightFX(lightType: UInt8) -> [NeewerLightFX] {
        ...
        {
            // Prefer fxPatterns from the database when available
            if let fxPatterns = item.fxPatterns, !fxPatterns.isEmpty {
                fxPatterns.forEach { (item) in
                    fxs.append(NeewerLightFX.parseNamedCmdToFX(item: item))
                }
            }
            else if item.support17FX ?? false
            {
